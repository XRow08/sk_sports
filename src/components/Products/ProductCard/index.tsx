  ...
  return (
    <Link href={`/products/${item.slug}`}>
      <div className="flex flex-col items-center w-[165px] lg:w-[305px] lg:h-[465px]">
        ...
        />
        <p className="text-sm lg:text-lg font-bold text-start w-full">{item.name}</p>
        <div className="flex justify-between w-full">
          <div className="flex items-baseline gap-1">
            <p className="font-extrabold text-sm lg:text-xl">{formatToBRL(item.price)}</p>
        ...
        <div className="flex justify-between w-full">
          <p className="font-bold text-sm lg:text-lg">{formatToBRL(discountValue)}</p>
